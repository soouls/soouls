    ...
    <div className="animate-slide-up space-y-8">
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { label: 'Waiting', value: queue.waiting, color: 'text-slate-300 bg-slate-400/10 border-slate-400/20' },
            { label: 'Active', value: queue.active, color: 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20' },
            { label: 'Delayed', value: queue.delayed, color: 'text-amber-300 bg-amber-400/10 border-amber-400/20' },
            { label: 'Failed', value: queue.failed, color: 'text-rose-400 bg-rose-400/10 border-rose-400/20' },
          ].map((item) => (
            <div
              key={item.label}
        ...
        <Panel title="WebSocket Connections">
          <div className="space-y-4">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-mono text-2xl font-bold text-white">{telemetry.websocketConnections}</span>
            </div>
          </div>
        </Panel>
        ...
        <Panel title="External Providers">
          <div className="space-y-3">
            <div className="flex items-center justify-between rounded-xl bg-white/[0.02] px-4 py-3">
              ...
              <StatusBadge status={messaging.providerHealth.emailConfigured ? 'active' : 'offline'} />
            </div>
            ...
            <div className="flex items-center justify-between rounded-xl bg-white/[0.02] px-4 py-3">
              ...
              <StatusBadge status={messaging.providerHealth.whatsappConfigured ? 'active' : 'offline'} />
            </div>
            ...
            <div className="flex items-center justify-between rounded-xl bg-white/[0.02] px-4 py-3">
              ...
              <StatusBadge status={messaging.providerHealth.newsletterConfigured ? 'active' : 'offline'} />
            </div>
          </div>
        </Panel>
      </div>
