...
export default function OverviewPage() {
  ...
  const canManageEngineering =
    viewer?.permissions?.includes('mutate:feature_flags');
    ...
    <div className="animate-slide-up space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <StatCard
          ...
          value={queueTotal}
          label="Queue Jobs"
          trend={overview.queue.failed > 0 ? `${overview.queue.failed} failed` : queueTotal === 0 ? 'Idle' : undefined}
        />
        <StatCard
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { label: 'Waiting', value: overview.queue.waiting, color: 'text-slate-300 bg-slate-400/10 border-slate-400/20' },
            { label: 'Active', value: overview.queue.active, color: 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20' },
            { label: 'Delayed', value: overview.queue.delayed, color: 'text-amber-300 bg-amber-400/10 border-amber-400/20' },
            { label: 'Failed', value: overview.queue.failed, color: 'text-rose-400 bg-rose-400/10 border-rose-400/20' },
          ].map((item) => (
            <div
              key={item.label}
            ...
            >
              ...
              <div className="mt-1 text-[10px] uppercase tracking-widest opacity-70">{item.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <Panel title="Feature Flags" action={
          <div className="flex items-center gap-1.5 text-xs text-slate-500">
            <ToggleRight className="h-3.5 w-3.5" />
            <span>{overview.featureFlags.filter(f => f.enabled).length}/{overview.featureFlags.length} enabled</span>
          </div>
        }>
      ...
      <Panel title="Emergency Kill Switches" action={
        <div className="flex items-center gap-1.5 text-xs">
          <span className={overview.serviceControls.every(c => c.enabled) ? 'text-emerald-400' : 'text-rose-400'}>
            {overview.serviceControls.filter(c => c.enabled).length}/{overview.serviceControls.length} online
          </span>
        </div>
      }>
