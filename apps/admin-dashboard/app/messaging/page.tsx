...
export default function MessagingPage() {
  ...
  const [composeBrand, setComposeBrand] = useState<'soulcanvas' | 'soulcanvas-studio' | 'founder-desk'>('soulcanvas');
        ...
        <Panel title="Compose Campaign">
          <div className="grid gap-6">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-xs font-medium text-slate-400">Internal Title</label>
                <input
                  value={composeTitle}
              ...
              <div>
                <label className="mb-1.5 block text-xs font-medium text-slate-400">Brand Persona</label>
                <select
                  value={composeBrand}
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-slate-400">Email Subject / Notification Header</label>
              <input
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-slate-400">Markdown Body</label>
              <textarea
                value={composeBody}
            ...
            <div className="flex items-center justify-between border-t border-white/[0.06] pt-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <ActionButton
                  variant="primary"
                  onClick={handleQueue}
                  disabled={!composeTitle || !composeSubject || !composeBody || composeChannels.length === 0}
                >
                  Queue Broadcast
                </ActionButton>
              </div>
          ...
          <div className="space-y-2 mt-4">
            {messaging.campaigns.length === 0 ? (
              ...
              messaging.campaigns.map((campaign) => (
                <div key={campaign.id} className="flex items-start justify-between rounded-xl bg-white/[0.02] px-5 py-4">
                  <div>
          ...
          <div className="space-y-4">
            {messaging.recentDeliveries.length === 0 ? (
              ...
              messaging.recentDeliveries.map((delivery) => (
                <div key={delivery.id} className="flex items-center justify-between border-l-2 border-amber-400/50 pl-3">
                  <div>
                    <div className="flex items-center gap-2">
