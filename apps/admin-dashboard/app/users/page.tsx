                      ...
                      <ActionButton
                        ...
                        onClick={() =>
                          void api(`/command-api/users/${selectedUser.id}/account-status`, {
                            method: 'PATCH',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ status: selectedUser.accountStatus === 'suspended' ? 'active' : 'suspended' }),
                          }).then(() => {
                            setFlash(selectedUser.accountStatus === 'suspended' ? 'User reactivated.' : 'User suspended.');
                            void api<UserProfile>(`/command-api/users/${selectedUser.id}`).then(setSelectedUser);
                          })
                        }
                      >
