      ...
      <div className="relative z-10 max-w-[1240px] mx-auto px-6">
        ...
        <div className="mb-16 md:mb-24 flex flex-col md:flex-row justify-between items-start md:items-end gap-8 reveal">
          <div className="max-w-2xl text-left">
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-[var(--soouls-accent)]/10 mb-6 transition-transform duration-300 active:scale-95 hover:rotate-12">
              <BookOpen className="w-6 h-6 text-[var(--soouls-accent)]" />
            </div>
            <p className="font-urbanist text-[var(--ink)] font-bold text-[var(--soouls-accent)] font-semibold tracking-tight mb-6 block">
              The Soouls Journal
            </p>
            <h1 className="font-playfair text-4xl sm:text-5xl md:text-7xl font-bold text-[var(--ink)] leading-[1.1] mb-6">
              Thoughts on <br className="hidden sm:block" /> <em className="italic text-[var(--soouls-accent)]">thinking</em>
            </h1>
          </div>
          <button
            type="button"
            className="group px-8 py-4 rounded-full border border-[var(--soouls-border)] bg-[rgba(var(--soouls-bg-elevated-rgb),0.8)] backdrop-blur-sm text-[var(--ink)] font-urbanist font-bold text-sm font-semibold tracking-tight hover:bg-[var(--ink)] hover:border-[var(--ink)] hover:text-[var(--paper)] transition-colors transition-transform transition-shadow duration-300 shadow-sm flex items-center gap-2"
          >
            View All Articles
        ...
        {/* Featured Post */}
        <div className="featured-post-container group relative w-full h-[400px] md:h-[600px] mb-16 rounded-3xl md:rounded-[3rem] overflow-hidden cursor-pointer reveal border border-[var(--border)]/20 shadow-xl">
          ...
          <div className="absolute bottom-0 left-0 p-8 sm:p-12 md:p-20 z-20 max-w-3xl transform group-hover:-translate-y-2 transition-transform duration-300">
            <div className="flex items-center gap-4 mb-6">
              <span className="font-urbanist text-xs font-bold text-[var(--soouls-accent)] font-semibold tracking-tight">
                Featured Thinking
              </span>
              <span className="flex items-center gap-1.5 font-urbanist text-[10px] text-white/60 font-bold font-semibold tracking-tight bg-[rgba(var(--soouls-bg-elevated-rgb),0.1)] px-3 py-1 rounded-full backdrop-blur-md">
                <Clock className="w-3 h-3" /> 10 min read
              </span>
            </div>
            
            <h3 className="font-playfair text-3xl sm:text-4xl md:text-6xl font-bold text-white mb-6 group-hover:text-[var(--paper)] transition-colors leading-tight animated-underline pb-2">
            ...
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-[rgba(var(--soouls-bg-elevated-rgb),0.2)] backdrop-blur-md border border-white/40 flex items-center justify-center overflow-hidden">
        ...
        <div className="blog-grid editorial-grid mb-20 md:mb-32">
          {posts.map((post, idx) => (
            ...
            >
              <div className={`relative ${idx === 0 ? 'h-[400px] md:h-[480px]' : 'h-[280px] md:h-[320px]'} rounded-3xl md:rounded-[2.5rem] overflow-hidden mb-6 border border-[var(--border)]/50 shadow-sm group-hover:shadow-[0_20px_40px_rgba(var(--soouls-accent-rgb),0.15)] transition-colors transition-transform transition-shadow duration-300`}>
              ...
              <div className="px-2">
                <div className="flex justify-between items-center mb-4">
                  <span className="inline-block px-3 py-1 bg-[var(--soouls-accent)]/10 rounded-full font-urbanist text-[10px] font-bold text-[var(--soouls-accent)] font-semibold tracking-tight transform group-hover:-translate-y-1 transition-transform duration-300">
                    {post.category}
                  </span>
        ...
        <div className="max-w-[1000px] mx-auto reveal" style={{ transitionDelay: '200ms' }}>
          <div className="relative p-8 sm:p-12 md:p-24 rounded-[2rem] md:rounded-[4rem] bg-gradient-to-br from-[var(--paper)]/80 to-white/60 backdrop-blur-3xl border border-[var(--border)]/60 overflow-hidden flex flex-col items-center text-center shadow-[0_8px_32px_rgba(var(--soouls-accent-rgb),0.06)] group">
            
            <div className="newsletter-glow absolute top-0 right-0 w-[400px] h-[400px] bg-[var(--soouls-accent)]/10 blur-[80px] rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none transition-opacity duration-500" />
            <div className="newsletter-glow absolute bottom-0 left-0 w-[300px] h-[300px] bg-[var(--secondary-accent)]/10 blur-[80px] rounded-full translate-y-1/2 -translate-x-1/2 pointer-events-none transition-opacity duration-500" style={{ animationDelay: '1.5s' }} />
            
            <div className="relative z-10 w-full flex flex-col items-center">
              <h3 className="font-playfair text-3xl sm:text-4xl md:text-5xl font-bold text-[var(--ink)] mb-6">
                The Weekly <em className="italic text-[var(--soouls-accent)]">Whisper</em>
              </h3>
              ...
              >
                <div className="relative flex-1">
                  <input
                    ...
                    className="w-full bg-[rgba(var(--soouls-bg-elevated-rgb),1)] border border-[var(--soouls-border)] rounded-full px-8 py-5 text-[var(--ink)] font-urbanist focus:outline-none focus:border-[var(--soouls-accent)] focus:ring-4 focus:ring-[rgba(var(--soouls-accent-rgb),0.1)] transition-colors transition-transform transition-shadow shadow-sm peer"
                    required
                  />
                  <div className="absolute inset-0 rounded-full border-2 border-transparent peer-focus:border-[rgba(var(--soouls-accent-rgb),0.3)] pointer-events-none transition-colors" />
                </div>
                <button
                  type="submit"
                  className="px-10 py-5 bg-[var(--ink)] text-[var(--paper)] font-urbanist font-bold text-sm font-semibold tracking-tight rounded-full hover:opacity-80 hover:scale-105 active:scale-[0.97] transition-colors transition-transform transition-shadow duration-300 shadow-[0_10px_20px_rgba(22,19,15,0.15)] whitespace-nowrap overflow-hidden relative group/submit"
                >
