      ...
      <div className="relative z-10 max-w-[1240px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20 md:mb-32">
          {features.map((feat, idx) => {
            ...
            return (
              <div 
                key={idx} 
                className="group relative bg-[#fdfaf6]/60 backdrop-blur-md border border-[#e3dbcd]/50 rounded-3xl md:rounded-[3rem] p-6 sm:p-8 lg:p-14 transition-all duration-700 hover:bg-white/90 hover:shadow-[0_30px_60px_rgba(217,138,75,0.08)] hover:-translate-y-2 reveal overflow-hidden"
                style={{ transitionDelay: `${(idx % 4) * 100}ms` }}
              >
                {/* Decorative Background Glows */}
                <div className={`absolute -top-32 -right-32 w-64 h-64 rounded-full blur-[80px] opacity-0 group-hover:opacity-40 transition-opacity duration-1000 pointer-events-none ${idx % 2 === 0 ? 'bg-[var(--soouls-accent)]' : 'bg-[#6450d6]'}`} />
                <div className={`absolute -bottom-32 -left-32 w-64 h-64 rounded-full blur-[80px] opacity-0 group-hover:opacity-30 transition-opacity duration-1000 pointer-events-none ${idx % 2 === 0 ? 'bg-[#E07A5F]' : 'bg-[var(--soouls-accent)]'}`} />
                
                <div className="relative z-10">
                  <div className="w-16 h-16 rounded-3xl bg-white border border-[#e3dbcd] shadow-sm flex items-center justify-center mb-10 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-500">
                    <Icon size={28} strokeWidth={1.5} className="text-[var(--ink)] group-hover:text-[var(--soouls-accent)] transition-colors duration-500" />
                  </div>
                  ...
                  <div className="inline-block p-3 bg-[var(--soouls-accent)]/5 rounded-xl border border-[var(--soouls-accent)]/10 mb-6 group-hover:bg-[var(--soouls-accent)]/10 transition-colors duration-500">
        ...
        <div className="max-w-[1000px] mx-auto reveal" style={{ transitionDelay: '300ms' }}>
          <div className="relative bg-gradient-to-br from-[var(--ink)] to-[#2a251e] border border-[#e3dbcd]/20 shadow-[0_20px_60px_rgba(22,19,15,0.3)] rounded-3xl md:rounded-[2.5rem] md:rounded-[4rem] p-8 sm:p-12 md:p-24 overflow-hidden group">
            ...
            <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#6450d6]/20 rounded-full blur-[100px] opacity-30 group-hover:opacity-60 group-hover:scale-110 transition-all duration-1000 pointer-events-none" />
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              ...
              <button className="group/btn relative px-10 py-5 bg-[var(--soouls-accent)] text-white rounded-full font-urbanist font-bold text-sm tracking-widest uppercase overflow-hidden shadow-[0_10px_30px_rgba(217,138,75,0.3)] hover:scale-105 transition-transform duration-300">
                <span className="relative z-10 flex items-center gap-2">
                  Start Writing <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform" />
                </span>
                <div className="absolute inset-0 bg-gradient-to-r from-[#E07A5F] to-[var(--soouls-accent)] opacity-0 group-hover/btn:opacity-100 transition-opacity duration-500" />
              </button>
            </div>
          </div>
        </div>
