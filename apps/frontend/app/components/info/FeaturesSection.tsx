    ...
    <section id="features" className="relative w-full py-10 bg-transparent overflow-hidden">
      <div className="relative z-10 max-w-[1240px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20 md:mb-32">
          {features.map((feat, idx) => {
            ...
            return (
              <div 
                key={idx} 
                className="group relative bg-[var(--paper)]/60 backdrop-blur-md border border-[var(--ink-soft)]/20 rounded-3xl md:rounded-[3rem] p-6 sm:p-8 lg:p-14 transition-colors transition-transform transition-shadow duration-300 hover:bg-[rgba(var(--soouls-bg-elevated-rgb),1)] hover:shadow-[0_30px_60px_rgba(217,138,75,0.08)] hover:-translate-y-2 reveal overflow-hidden"
                style={{ transitionDelay: `${(idx % 4) * 100}ms` }}
              >
                {/* Decorative Background Glows */}
                <div className={`absolute -top-32 -right-32 w-64 h-64 rounded-full blur-[80px] opacity-0 group-hover:opacity-40 transition-opacity duration-500 pointer-events-none ${idx % 2 === 0 ? 'bg-[var(--soouls-accent)]' : 'bg-[var(--secondary-accent)]'}`} />
                <div className={`absolute -bottom-32 -left-32 w-64 h-64 rounded-full blur-[80px] opacity-0 group-hover:opacity-30 transition-opacity duration-500 pointer-events-none ${idx % 2 === 0 ? 'bg-[var(--tertiary-accent)]' : 'bg-[var(--soouls-accent)]'}`} />
                
                <div className="relative z-10">
                  <div className="w-16 h-16 rounded-3xl bg-[rgba(var(--soouls-bg-elevated-rgb),1)] border border-[var(--soouls-border)] shadow-sm flex items-center justify-center mb-10 group-active:scale-95 group-hover:rotate-3 transition-transform duration-300">
        ...
        <div className="max-w-[1000px] mx-auto reveal" style={{ transitionDelay: '300ms' }}>
          <div className="relative bg-gradient-to-br from-[var(--ink)] to-[#2a251e] border border-[var(--soouls-border)] shadow-[0_20px_60px_rgba(22,19,15,0.3)] rounded-[2rem] md:rounded-[4rem] p-8 sm:p-12 md:p-24 overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              ...
              <h3 className="font-playfair text-3xl sm:text-4xl md:text-6xl font-bold text-[var(--paper)] leading-tight mb-8">
              ...
              <Link href="/sign-up" className="group/btn relative px-10 py-5 inline-block bg-[var(--soouls-accent)] text-white rounded-full font-urbanist font-bold text-sm font-semibold tracking-tight overflow-hidden shadow-[0_10px_30px_rgba(217,138,75,0.3)] active:scale-[0.97] transition-all duration-200">
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-[var(--soouls-accent)] to-[var(--soouls-accent)] opacity-0 group-hover/btn:opacity-100 transition-opacity duration-300" />
              </Link>
            </div>
          </div>
        </div>
