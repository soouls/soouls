  ...
  const categories = [
    {
      ...
      icon: GridIcon,
      color: 'text-[var(--soouls-accent)]',
      bgHover: 'hover:border-[var(--soouls-accent)]',
    },
    {
      id: 'account',
      title: 'Account',
      desc: 'Sign in, sync, delete',
      icon: UserCircleIcon,
      color: 'text-[#6450d6]',
      bgHover: 'hover:border-[#6450d6]',
    },
    {
      id: 'billing',
      title: 'Billing',
      desc: 'Plans, cancel, refunds',
      icon: CreditCardIcon,
      color: 'text-[#E07A5F]',
      bgHover: 'hover:border-[#E07A5F]',
    },
    {
      id: 'privacy',
      title: 'Privacy & data',
      desc: 'Encryption, export',
      icon: LockIcon,
      color: 'text-[#4ea896]',
      bgHover: 'hover:border-[#4ea896]',
    },
  ];
  ...
  const faqs = [
    {
      id: 'acc-1',
      category: 'account',
      q: 'How do I sync my entries across devices?',
      a: 'Signing in with the same account on web, iOS, and Android automatically syncs your entries. Sync happens in the background whenever you have an internet connection.',
    },
    {
      id: 'acc-2',
      category: 'account',
      q: 'Can I change my email address?',
      a: "Yes, from Settings → Account. You'll need to verify the new address before the change takes effect.",
    },
    {
      id: 'acc-3',
      category: 'account',
      q: 'How do I permanently delete my account?',
      a: 'Go to Account → Data & Ownership → Delete account. This is permanent and removes your entries from active systems within 30 days. We recommend exporting your data first.',
    },
    {
      id: 'bil-1',
      category: 'billing',
      q: 'How do I cancel my subscription?',
      a: 'Settings → Billing → Cancel plan. Your access continues until the end of the current billing period, and your entries are never affected.',
    },
    {
      id: 'bil-2',
      category: 'billing',
      q: 'What happens to my entries if I downgrade?',
      a: "Nothing is deleted. You'll lose access to features tied to the higher tier, like Sunday Review or unlimited clusters, but every entry stays readable and exportable.",
    },
    {
      id: 'bil-3',
      category: 'billing',
      q: 'Do you offer refunds?',
      a: 'Yes — a full refund within 14 days of your first payment on any paid plan, no questions asked. Contact support to request one.',
    },
    {
      id: 'pri-1',
      category: 'privacy',
      q: 'Is my journal actually private?',
      a: 'Yes. Entries are encrypted at rest and in transit, and access to production data is restricted and audit-logged. We never sell your data or use it for advertising.',
    },
    {
      id: 'pri-2',
      category: 'privacy',
      q: 'Are my entries used to train any AI model?',
      a: 'No, on any plan, ever. Insight generation processes your entries to serve you personally and is never retained to train models.',
    },
    {
      id: 'pri-3',
      category: 'privacy',
      q: 'How do I export all of my data?',
      a: "Settings → Data & Ownership → Download your data. You'll get a complete export in an open, portable format within a few minutes.",
    },
  ];
  ...
  return (
    <section className="relative w-full py-10 bg-transparent overflow-hidden">
      <div className="relative z-10 max-w-[900px] mx-auto">
        ...
        {!searchQuery && (
          <div
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-20 reveal"
          ...
          >
            {categories.map((cat, _idx) => {
              ...
              return (
                ...
                >
                  <div
                    className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-2xl opacity-20 pointer-events-none transition-opacity duration-300 ${isActive ? 'bg-white opacity-10' : cat.color.replace('text-', 'bg-')} group-hover:opacity-30`}
                  />

                  <div
                    className={`relative z-10 w-12 h-12 rounded-full flex items-center justify-center mb-6 transition-transform duration-300 group-active:scale-95 ${
                  ...
                  <h4 className="relative z-10 font-playfair font-bold text-xl mb-2">
                    {cat.title}
                  </h4>
                  <p
                    className={`relative z-10 font-urbanist text-sm leading-snug ${isActive ? 'text-white/70' : 'text-[var(--ink-faint)]'}`}
                  >
                    {cat.desc}
                  </p>
                </button>
          ...
          {filteredFaqs.length === 0 ? (
            <div className="text-center py-20 bg-[#fdfaf6]/60 backdrop-blur-md border border-[#e3dbcd]/50 rounded-3xl md:rounded-[3rem] font-urbanist text-lg text-[var(--ink-soft)] shadow-inner">
              ...
              <span className="text-sm mt-2 block">
                Try a different term or contact support below.
              </span>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-6 pl-2">
                      <div
                        className={`w-2 h-2 rounded-full ${catInfo?.color.replace('text-', 'bg-')}`}
                      />
                      <h3
                        className={`font-urbanist text-xs font-bold font-semibold tracking-tight ${catInfo?.color}`}
                      >
                        {catInfo?.title}
                      </h3>
                    </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              <div
                                className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition-colors transition-transform transition-shadow duration-300 ${
                                  isOpen
        ...
        {/* CTA */}
        <div
          className="mt-24 max-w-4xl mx-auto text-center reveal"
        ...
        >
          <div className="relative bg-gradient-to-br from-[#fdfaf6]/80 to-[#fdfaf6]/40 backdrop-blur-3xl border border-[#e3dbcd]/60 shadow-[0_8px_32px_rgba(100,80,214,0.06)] rounded-3xl md:rounded-[3rem] p-8 sm:p-12 md:p-20 overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <a
                href="mailto:support@soouls.in"
                className="inline-block px-10 py-4 bg-[var(--ink)] text-[#f7f3ec] rounded-full font-urbanist font-bold text-sm font-semibold tracking-tight active:scale-[0.97] transition-all duration-200"
              >
                Contact support
              </a>
            </div>
