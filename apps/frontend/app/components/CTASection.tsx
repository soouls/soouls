  ...
  return (
    <section className="py-24 px-6 relative z-10 flex justify-center">
      <div className="bg-[#e6e2f8] relative overflow-hidden w-full max-w-6xl rounded-[2rem] md:rounded-[3rem] py-16 px-6 md:py-28 md:px-8 flex flex-col items-center text-center shadow-sm">
        ...
        <span className="text-[#6450d6] text-[10px] font-bold tracking-[0.2em] uppercase mb-8 z-10">
          Start Now
        </span>

        <h2 className="text-3xl sm:text-4xl md:text-6xl font-bold text-[#16130f] max-w-3xl leading-[1.1] mb-6 tracking-tight z-10">
          Your mind has been waiting for a page that{' '}
          <em className="font-playfair font-normal italic text-[#6450d6]">moves</em> like it does
        </h2>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-14 z-10 w-full max-w-2xl">
          <div className="flex items-center gap-2 bg-white/70 px-4 py-2.5 rounded-xl border border-white shadow-sm backdrop-blur-md transition-transform hover:-translate-y-1 cursor-default">
            <span className="relative flex h-2.5 w-2.5 mr-1">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75" />
              <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-green-500" />
            </span>
            <svg
              className="w-4 h-4 text-[#16130f]"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
        ...
        <div className="flex flex-col items-center z-10">
          <p className="text-[#4a4237] text-sm mb-6 font-medium">
            Still deciding? Ask an AI what it thinks of Soouls.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <button
              type="button"
              className="bg-white px-6 py-3 rounded-full shadow-sm text-sm font-semibold text-[#16130f] flex items-center gap-3 hover:-translate-y-1 transition-transform"
            >
          ...
          <p className="text-[10px] text-[#4a4237]/50 mt-5 tracking-wide">
            Opens in a new tab — no account needed to ask.
          </p>
        </div>
      </div>
    </section>
