...
function SearchPopup({ entries, onClose }: { entries: UserEntry[]; onClose: () => void }) {
  ...
  return (
    <motion.div
      className="fixed inset-0 z-[70] flex items-end justify-start bg-black/30 p-4 backdrop-blur-sm sm:p-8"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    >
      <motion.div
        className="relative grid w-full max-w-[986px] grid-cols-1 gap-6 rounded-[16px] bg-[rgba(14,14,14,0.92)] p-6 shadow-2xl backdrop-blur-[30px] md:grid-cols-[1.2fr_0.9fr] md:p-10"
          ...
          <div className="space-y-4">
            {(filtered.length ? filtered : entries.slice(0, 4)).map((entry, index) => (
              <button
                ...
                className="flex w-full items-center justify-between rounded-2xl border border-[#222] bg-[rgba(15,15,15,0.5)] px-7 py-4 text-left transition-colors hover:border-[#333]"
              >
            ...
            {!entries.length ? (
              <p className="rounded-2xl border border-[#222] px-6 py-5 text-[#7a7a7a]">
                Your recent entries will appear here after you write.
              </p>
            ) : null}
          </div>
        </div>
...
export default function HomePage() {
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {isSearchOpen && (
          <SearchPopup entries={entries?.items ?? []} onClose={() => setIsSearchOpen(false)} />
        )}
      </AnimatePresence>
      <AnimatePresence>
