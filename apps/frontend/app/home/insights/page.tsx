...
import { Sparkles, Calendar, Moon, TrendingUp, TrendingDown, Target, Zap } from "lucide-react";
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-6 relative z-10 flex flex-col pt-4 pb-20">
        ...
        <div className="bg-[#141414]/90 backdrop-blur-2xl border border-white/5 rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-4">
            <h2 className="text-2xl font-medium tracking-tight text-white/90">Soulcanvas Insights</h2>
            ...
            <div className="flex gap-4 mb-6">
              ...
              <div className="space-y-4">
                <blockquote className="text-3xl md:text-4xl font-serif italic leading-[1.3] text-white/90">
                  "This month, your mind gravitated toward <span className="text-[#FF5C35]">turning ideas into action.</span> There's <span className="text-[#FF9E80]">a clear transition</span> from scattered inspiration to focused execution."
                </blockquote>
                <p className="text-white/40 text-sm leading-relaxed max-w-2xl font-light">
                  Your activity reflects a <span className="text-white/70 font-medium">35% increase</span> in goal-oriented thinking compared to previous weeks. The recurring theme of "Exploration" has matured into "Clarity," indicating you're beginning to define your direction with confidence.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-white/[0.03] border border-white/5 p-8 rounded-3xl">
              <h3 className="text-sm font-semibold tracking-wider text-white/60 mb-8 uppercase">Thought Themes</h3>
            ...
            {/* Reflection Patterns */}
            <div className="bg-white/[0.03] border border-white/5 p-8 rounded-3xl">
              <h3 className="text-sm font-semibold tracking-wider text-white/60 mb-6 uppercase">Reflection Patterns</h3>
              <div className="flex gap-4 mb-10">
                <Moon className="w-5 h-5 text-[#FF9E80] shrink-0" />
                <p className="text-xs text-white/50 leading-relaxed italic">
                  You tend to reflect most during late-evenings <span className="text-white/80">(10 PM – 12 AM)</span>, when your thoughts are more structured yet introspective.
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
            ...
            <div className="bg-white/[0.03] border border-white/5 p-8 rounded-3xl flex flex-col">
              <h3 className="text-sm font-semibold tracking-wider text-white/60 mb-2 uppercase">How your Thoughts connect</h3>
              <span className="text-[10px] text-white/20 tracking-widest mb-10 uppercase">RELATIONS BY SIMILARITY</span>
              <div className="flex-1 flex items-center justify-center relative min-h-[160px]">
                 {/* Mocking the node map from screenshot */}
                 <div className="absolute w-2 h-2 bg-[#FF5C35] rounded-full shadow-[0_0_10px_#FF5C35]" style={{top: '20%', left: '20%'}} />
                 <div className="absolute w-3 h-3 bg-white/40 rounded-full" style={{top: '40%', left: '50%'}} />
                 <div className="absolute w-2 h-2 bg-white/20 rounded-full" style={{top: '70%', left: '30%'}} />
                 <div className="absolute w-2.5 h-2.5 bg-[#FF9E80] rounded-full shadow-[0_0_8px_#FF9E80]" style={{top: '60%', left: '80%'}} />
            ...
            {/* Shifting Metrics */}
            <div className="bg-white/[0.03] border border-white/5 p-8 rounded-3xl">
              <h3 className="text-sm font-semibold tracking-wider text-white/60 mb-2 uppercase">Your thinking is shifting</h3>
              <span className="text-[10px] text-white/20 tracking-widest mb-10 uppercase">EVOLUTION STATUS</span>
              <div className="space-y-6">
                {shiftMetrics.map((item) => (
                  <div key={item.label} className="flex justify-between items-center">
                    <span className="text-[10px] tracking-wider text-white/40 font-bold uppercase">{item.label}</span>
                    <div className="flex items-center gap-2">
                      {item.status === 'down' && <><TrendingDown className="w-3 h-3 text-[#FF5C35]" /> <span className="text-xs text-white/70">{item.value}%</span></>}
                      {item.status === 'up' && <><TrendingUp className="w-3 h-3 text-emerald-500" /> <span className="text-xs text-white/70">+{item.value}%</span></>}
                      {item.status === 'badge' && <span className="text-[9px] bg-[#FF5C35]/20 text-[#FF5C35] px-2 py-0.5 rounded border border-[#FF5C35]/30 font-bold tracking-widest">{item.value}</span>}
          ...
          <div className="pt-12 border-t border-white/5 mt-8">
            <div className="flex flex-col items-center text-center max-w-2xl mx-auto">
              ...
              <span className="text-[10px] font-bold tracking-[0.3em] text-[#FF5C35] mb-4 uppercase">Final Synthesis</span>
              ...
              <p className="text-white/40 text-sm leading-relaxed font-light italic">
                The data suggests your mental architecture is shifting towards long-term legacy rather than short-term gains. This synthesis marks the end of your "Exploration" phase.
              </p>
            </div>
          </div>
