...
const DashboardPage = () => {
  ...
  const weeklyBars = useMemo(() => {
    ...
    entries.filter(e => new Date(e.createdAt) >= weekAgo).forEach(e => {
  ...
  const lastMonthEntries = entries.filter(e => new Date(e.createdAt).getMonth() === (now.getMonth() - 1 + 12) % 12).length;
  const growthPercent = lastMonthEntries === 0 ? (currentMonthEntries > 0 ? 100 : 0) : Math.round(((currentMonthEntries - lastMonthEntries) / lastMonthEntries) * 100);
        ...
        >
          <div className="flex-1 p-8 md:p-10 space-y-10 overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-6">
              ...
              <div className="w-full bg-[#111111] border border-white/[0.05] rounded-[24px] p-8 flex flex-col min-h-[220px] transition-all group relative cursor-text" onClick={() => router.push('/home/new-entry')}>
                <span className="text-[24px] text-white/20 font-light mb-auto">Write something to start</span>
                <div className="flex items-center justify-between mt-10">
                  <div className="flex items-center gap-10">
                    <button className="flex items-center gap-2.5 text-[#A78BFA] hover:opacity-80 transition-all">
                      <Mic className="w-5 h-5" />
                      <span className="text-[16px] font-medium">Voice note</span>
                    </button>
                    <button className="flex items-center gap-2.5 text-[#E07A5F] hover:opacity-80 transition-all">
                      <ImageIcon className="w-5 h-5" />
                      <span className="text-[16px] font-medium">Image</span>
                    </button>
                    <button className="flex items-center gap-2.5 text-[#34D399] hover:opacity-80 transition-all">
                      <CheckSquare className="w-5 h-5" />
                      <span className="text-[16px] font-medium">Task</span>
                    </button>
                  </div>
                  <button 
                    onClick={(e) => { e.stopPropagation(); router.push('/home/new-entry'); }}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-[#111111] border border-white/[0.05] rounded-[24px] p-8 flex flex-col gap-6 relative overflow-hidden group cursor-pointer hover:border-white/10 transition-all">
                  <div className="flex items-center justify-between">
                    <h3 className="text-[18px] font-medium text-white/90">{bestCluster?.name || "The Midnight Echos"}</h3>
                  ...
                  <div className="flex-1 rounded-[20px] bg-[#0A0A0A] border border-white/[0.03] relative p-6 overflow-hidden min-h-[350px]">
                    ...
                    {clusterEntries.map((entry, i) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`text-[6px] mb-0.5 uppercase font-bold ${i === 0 ? 'text-[#E07A5F]' : 'text-white/30'}`}>
                            {new Date(entry.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                          </div>
                          <div className={`text-[10px] font-bold mb-0.5 truncate ${i === 0 ? 'text-white' : 'text-white/70'}`}>
                            {getEntryTitle(entry)}
                          </div>
                    ...
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 w-full px-4">
                      <div className="flex items-center gap-2 p-1 rounded-full bg-black/60 border border-white/10 backdrop-blur-md">
                        <button className="px-3 py-1 text-[10px] text-white/40 uppercase font-bold hover:text-white transition-colors flex items-center gap-1">
                          <Plus className="w-2.5 h-2.5" /> CREATE
                        </button>
                        <div className="w-[1px] h-3 bg-white/10" />
                        <div className="px-3 py-1 text-[10px] text-[#E07A5F] uppercase font-bold">CD</div>
                        <div className="w-[1px] h-3 bg-white/10" />
                        <button className="p-1 text-white/40 hover:text-white transition-colors"><LayoutGrid className="w-3.5 h-3.5" /></button>
                        <div className="w-[1px] h-3 bg-white/10" />
                        <button className="p-1 text-white/40 hover:text-white transition-colors"><Plus className="w-3.5 h-3.5" /></button>
                      </div>
                      <button className="flex items-center gap-2 px-5 py-2.5 bg-[#E07A5F]/10 border border-[#E07A5F]/30 rounded-full text-[#E07A5F] text-[12px] font-bold hover:bg-[#E07A5F] hover:text-white transition-all">
                ...
                <div className="bg-[#111111] border border-white/[0.05] rounded-[24px] p-8 flex flex-col">
                  ...
                  <button className="text-[#E07A5F] text-[14px] font-medium hover:underline text-left mt-auto" onClick={() => router.push('/home/entries')}>View all Tasks</button>
                </div>
                  ...
                  <div className="flex items-center gap-8 relative z-10 mb-6">
                    ...
                    <div className="space-y-2">
                      <h2 className="text-[28px] leading-tight text-white/95 font-medium">
                        You reflected <span className="text-[#E07A5F] font-bold">{weeklyReflections} times</span> this week.
                      </h2>
                      <h3 className="text-[22px] text-white/80">
                        Top pattern detected: <span className="text-[#E07A5F] font-bold italic">"{topTheme}"</span>
                      </h3>
                    </div>
                  </div>
                ...
                <div className="bg-[#111111] border border-white/[0.05] rounded-[24px] p-8 flex flex-col gap-8 flex-1">
                  ...
                  <div className="flex-1 flex flex-col justify-center gap-6">
                    <p className="text-[20px] font-urbanist italic leading-[1.6] text-white/50 font-light">
                      {firstEntry ? `"${getEntryPlainText(firstEntry).slice(0, 200)}..."` : '"Start your journey by writing your first entry. Every thought counts towards your personal growth."'}
                    </p>
                    <div className="text-[24px] font-bold text-[#E07A5F] uppercase tracking-[0.1em]">
                      {firstEntry ? new Date(firstEntry.createdAt).toDateString() === new Date().toDateString() ? 'Today' : new Date(firstEntry.createdAt).toLocaleDateString('en-US', { weekday: 'long' }) : 'Ready'}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-end justify-between h-[180px] px-2 gap-3">
                      {weeklyBars.map((bar, idx) => (
                        <div key={bar.label} className="flex-1 flex flex-col items-center gap-4 h-full group">
                          ...
                          <span className={`text-[10px] font-bold uppercase tracking-tighter ${idx === now.getDay() ? 'text-[#E07A5F]' : 'text-white/20'}`}>{bar.label}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center gap-3 py-2.5 px-5 bg-[#E07A5F]/20 border border-[#E07A5F]/30 rounded-full justify-center">
                      ...
                      <span className="text-[14px] font-bold text-[#E07A5F]">{weeklyReflections} entries this week</span>
                    </div>
                  </div>
                </div>
              </div>
