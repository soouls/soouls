      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/[0.08]">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={onClose}
            aria-label="Close"
            className="p-2 rounded-xl hover:bg-[#e07a5f]/10 text-slate-400 hover:text-[#e07a5f] hover:drop-shadow-[0_0_8px_rgba(224,122,95,0.5)] transition-all duration-300"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
...
const GhostBtn = ({ onClick, children }: { onClick: () => void; children: React.ReactNode }) => (
  <button
    onClick={onClick}
    className="px-4 py-2 text-sm text-slate-400 hover:text-[#e07a5f] hover:drop-shadow-[0_0_8px_rgba(224,122,95,0.5)] transition-all duration-300"
  >
    {children}
  </button>
);
  ...
  return (
    <Overlay>
      <Modal
        ...
        extra={
          <>
            <IconBtn onClick={undo} aria-label="Undo doodle">
              <Undo2 className="w-4 h-4" />
            </IconBtn>
            <IconBtn onClick={clearCanvas} aria-label="Clear canvas">
      ...
      >
        <div className="flex items-center gap-3 px-5 py-2.5 border-b border-white/5 flex-wrap">
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-xl text-xs transition-all duration-300 ${tool === 'eraser' ? 'bg-[#e07a5f]/20 text-[#e07a5f]' : 'text-slate-400 hover:text-[#e07a5f] hover:drop-shadow-[0_0_8px_rgba(224,122,95,0.5)]'}`}
          >
    ...
    <Card onRemove={onRemove}>
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          aria-label={b.running ? 'Pause timer' : 'Play timer'}
          className="p-1 rounded-lg hover:bg-[#e07a5f]/10 text-slate-300 transition-colors"
        >
        ...
        <button
          ...
          className="p-1 rounded-lg hover:bg-[#e07a5f]/10 text-slate-400 transition-colors"
        >
...
function ToolBtn({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 px-5 py-4 transition-colors text-sm whitespace-nowrap flex-1 justify-center ${active ? 'bg-red-500/[0.08] text-red-400' : 'hover:bg-[#e07a5f]/10 text-slate-300'}`}
    >
      {icon}
      <span>{label}</span>
      {!!count && count > 0 && (
