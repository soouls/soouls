...
function versionSatisfies(version: string, range: string) {
  ...
  const [versionMajor = 0, versionMinor = 0] = version
    .split('-')[0]
    ?.split('.')
  ...
  if (prefix === '~') {
    return (
      versionMajor === major &&
      versionMinor === minor &&
      compareVersions(version, expected) >= 0
    );
  }
...
function ensureDependencyTree(
  ...
  const linkedDependency =
    existsSync(localDependency) ? localDependency : ensureBackendRuntimeDependency(packageName);
...
const backendManifest = readPackageManifest(appRoot) ?? readPackageManifest(dirname(backendManifestPath));
for (const [dependencyName, dependencyRange] of Object.entries(backendManifest?.dependencies ?? {})) {
  if (dependencyRange.startsWith('workspace:')) {
    continue;
  }
