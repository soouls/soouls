...
export class CommandCenterService {
  ...
  async ensureAuthorizedAdmin(clerkId: string, ipAddress?: string | null) {
    ...
    if (!isBootstrapAdmin) {
      console.log(`[CommandCenterAuth] Current bootstrap list: ${Array.from(this.bootstrapSuperAdmins).join(', ')}`);
    }
  ...
  async inviteAdmin(
    ...
    const grantedPermissions = (input.permissions ?? ['view:all']).filter(
